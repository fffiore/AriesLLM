...
class Model {
 public:
  ...
  virtual base::Status predict(const tensor::Tensor& input, const tensor::Tensor& pos_tensor,
                               const std::vector<int32_t>& history, bool is_prompt, int& next) const = 0;
  ...
  virtual std::pair<tensor::Tensor, tensor::Tensor> slice_kv_cache(int32_t layer_idx,
                                                                   int32_t token_pos, int32_t len) const;
 ...
 protected:
  ...
  virtual int32_t post_processing(const tensor::Tensor& pos, const std::vector<int32_t>& history, bool is_prompt) const = 0;
