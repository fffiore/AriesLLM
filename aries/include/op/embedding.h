...
struct EmbeddingOutput {
  ...
  explicit EmbeddingOutput(tensor::Tensor input_tokens, tensor::Tensor input_embeddings,
                           tensor::Tensor input_token_num)
      : input_tokens(std::move(input_tokens)),          // 输入的 tokens id
        input_embeddings(std::move(input_embeddings)),  // embedding 维度信息
        input_token_num(std::move(input_token_num)) {}  // 输入的 token 数量
};
