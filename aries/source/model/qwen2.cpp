...
base::Status Qwen2Model::init(base::DeviceType device_type) {
  ...
  if (device_type == DeviceType::kDeviceCUDA) {
    ...
    if (set_device_err != cudaSuccess) {
        return error::InternalError(std::string("cudaSetDevice(0) failed: ") + 
  ...
  max_position_embeddings_ = 32768;
  rotary_dim_ = config_->head_size_;
  init_mem();
  if (device_type_ == base::DeviceType::kDeviceCPU) {
    kernel::sin_cos_cache_calc_cpu(rotary_dim_, max_position_embeddings_, get_buffer(ModelBufferType::kSinCache),  
  ...
  config.temperature = 0.0f;            // temperature 越低越理性
  config.repetition_penalty = 1.0f;    // 1.0 表示无惩罚，数值越大，惩罚越多
  config.top_p = 1.0f;                  // 限制累积概率
  config.top_k = 40;                    // 限制前 40 个
  sampler_ = std::make_unique<sampler::SamplingSampler>(device_type_, config);
...
base::Status Qwen2Model::forward(const tensor::Tensor& input,
                                 ...
                                 int& next) const {
    bool is_decode = (input.get_dim(0) == 1);
    bool is_capturing_now = false;
    prompt_len_ = input.get_dim(0);
    bool use_graph = enable_cuda_graph_ && is_decode && device_type_ == base::DeviceType::kDeviceCUDA;
     // cuda graph capture 开启后不能进行 malloc 和 free 的工作
    if (device_type_ == base::DeviceType::kDeviceCUDA 
                && pos_tensor.device_type() != base::DeviceType::kDeviceCUDA) {
      // prefill 时如果传入的是 CPU tensor，需要临时拷到 GPU 给 RoPE kernel 用
      pos_ptr_ = pos_tensor.clone();
      pos_ptr_.to_cuda();
    }
    if (is_decode) {
      // 拷贝 pos_tensor 到固定 GPU buffer
      cudaMemcpy(pos_tensor_cu_.ptr<int32_t>(),
                pos_tensor.ptr<int32_t>(),
                sizeof(int32_t) * pos_tensor.size(),
                cudaMemcpyHostToDevice);
      pos_ptr_ = pos_tensor_cu_;
      if (use_graph && !graph_captured_) {
        cudaStreamBeginCapture(cuda_config_->stream, cudaStreamCaptureModeGlobal);
        is_capturing_now = true;
      }
      else if (use_graph && graph_captured_) {
        cudaGraphLaunch(graph_exec_, cuda_config_->stream);
    ...
    if (is_capturing_now) {
        cudaStreamEndCapture(cuda_config_->stream, &graph_);
...
void Qwen2Model::create_nonparam_layers() {
  ...
  qwen_layers_->rope_layer_ = std::make_shared<op::RoPELayer>( device_type_, config_->hidden_dim_, 
    config_->kv_dim_, config_->head_size_, rotary_dim_, max_position_embeddings_);
...
void Qwen2Model::init_mem() {
    ...
    int32_t seq_len = config_->seq_len_;
    int32_t dim = config_->dim_;
    int32_t hidden_dim = config_->hidden_dim_;
    int32_t head_num = config_->head_num_;
    ...
    tensor::Tensor input_tokens(
        base::DataType::kDataTypeInt32, seq_len, true, alloc_cpu);
    tensor::Tensor pos_tensor(
        base::DataType::kDataTypeInt32, seq_len, true, alloc_cpu);

    // embeddings
    tensor::Tensor input_embeddings(
        base::DataType::kDataTypeFp32, seq_len, dim, true, alloc);

    // RoPE
    tensor::Tensor sin_cache(
        base::DataType::kDataTypeFp32, seq_len, head_size, true, alloc);
    ...
    // RMSNorm, MHA output, W2 output, FFN norm
    ...
    tensor::Tensor w1_output(
        base::DataType::kDataTypeFp32, seq_len, hidden_dim, true, alloc);
    ...
    tensor::Tensor key_cache(
        base::DataType::kDataTypeFp32, layer_num, seq_len, kv_dim, true, alloc);
    ...
    // Query output and attn output share same shape [seq_len, dim]
    tensor::Tensor query(
        base::DataType::kDataTypeFp32, seq_len, dim, true, alloc);
    CHECK(insert_buffer(ModelBufferType::kQuery, query));
    CHECK(insert_buffer(ModelBufferType::kAttnOutput, query));

    // Attention score storage: [num_heads, seq_len]
    tensor::Tensor attn(
        base::DataType::kDataTypeFp32, head_num, seq_len, true, alloc);
    ...
    if (device_type_ == base::DeviceType::kDeviceCUDA) {
      ...
      // 分配临时 kv 存储 [2, seq_len = 1, kv_dim]
      temp_kv_storage_ = tensor::Tensor(base::DataType::kDataTypeFp32, 2 * config_->kv_dim_, true, alloc);
      // cuda graph capture 期间固定 buffer
      pos_tensor_cu_ = tensor::Tensor(base::DataType::kDataTypeInt32, 1, true, alloc);
    }
}
...
void Qwen2Model::attention_rms(int32_t layer_idx, const tensor::Tensor& input) const {
  ...
  // 在 prefill 时，input.shape(0) > 1
  // 确保输出 tensor 的形状与输入一致，否则后续计算会出错
  rmsnorm_output.reshape(input.dims());
...
void Qwen2Model::attention_qkv(int32_t layer_idx, const tensor::Tensor& pos_tensor) const {
    ...
    rmsnorm_output.reshape({prompt_len_, config_->dim_});

    tensor::Tensor query = this->get_buffer(ModelBufferType::kQuery); 
    query.reshape({prompt_len_, config_->dim_});

    tensor::Tensor key, val;
    if (prompt_len_ == 1 && device_type_ == base::DeviceType::kDeviceCUDA) {
      float* ptr_base = const_cast<float*>(temp_kv_storage_.ptr<float>());
      // kv cache 固定，根据 pos 确定写入位置
      key = tensor::Tensor(base::DataType::kDataTypeFp32, 1, config_->kv_dim_, false, nullptr, ptr_base);
      val = tensor::Tensor(base::DataType::kDataTypeFp32, 1, config_->kv_dim_, false, nullptr, ptr_base + config_->kv_dim_);
      key.set_device_type(device_type_);
      val.set_device_type(device_type_);
    } else {
      // key/val 指向 kv cache，后续的 RoPE 层把临时 key/val copy 到 kv cache
      int32_t start_pos = 0; 
      auto kv_pair = slice_kv_cache(layer_idx, start_pos, prompt_len_);
      key = kv_pair.first;
      val = kv_pair.second;
    }
    ...
    auto rope_layer = std::dynamic_pointer_cast<op::RoPELayer>(qwen_layers_->rope_layer_);
    // 执行 RoPE
    tensor::Tensor k_cache = get_buffer(ModelBufferType::kKeyCache);
    tensor::Tensor v_cache = get_buffer(ModelBufferType::kValueCache);
    rope_layer->forward(query, key, val, pos_ptr_,
                        get_buffer(ModelBufferType::kSinCache),
...
base::Status Qwen2Model::predict(const tensor::Tensor& input, const tensor::Tensor& pos_tensor,
                                 const std::vector<int32_t>& history, bool is_prompt, int& next) const {
  auto status = forward(input, pos_tensor, next);
  if (!status) {
    return status;
  }
  next = post_processing(pos_tensor, history, is_prompt);
  return base::error::Success();
}
...
void Qwen2Model::attention_mha(int32_t layer_idx, const tensor::Tensor& pos_tensor) const {
  ...
  // 不依赖 query.get_dim(0)，防止 attention_qkv 那边没 reshape 成功
  query.reshape({prompt_len_, config_->dim_});
  mha_output.reshape({prompt_len_, config_->dim_}); 
  
  // 最终 wo 投影输出
  tensor::Tensor attn_output = get_buffer(ModelBufferType::kAttnOutput);
  attn_output.reshape({prompt_len_, config_->dim_});

  // 调用层
  const auto& mha_layer = qwen_layers_->mha_layer_;
  CHECK_NE(mha_layer, nullptr) << "The multi head attention layer is null pointer.";
  // 有效 kv 范围 = [0 : pos + seq_len]，prefill 时这里是 start_pos (0)
  int pos = pos_tensor.index<int32_t>(0);
  // 历史序列长度 pos + seq_len
  std::dynamic_pointer_cast<op::MultiHeadAttention>(mha_layer)->set_pos(pos);
  // 当前 query 包含 [pos, pos + 1, ... ]
  std::dynamic_pointer_cast<op::MultiHeadAttention>(mha_layer)->set_layer_idx(layer_idx);
  
  auto mha_layer_ptr = std::dynamic_pointer_cast<op::MultiHeadAttention>(qwen_layers_->mha_layer_);
  CHECK_NE(mha_layer_ptr, nullptr) << "The multi head attention layer is null pointer.";
  // 设置 layer_idx
  mha_layer_ptr->set_layer_idx(layer_idx);
  // score_storage 在 prefill kernel 中未使用(使用 shared mem)，所以不需要 reshape 到 n * n
  STATUS_CHECK(mha_layer_ptr->forward(query, score_storage, key_cache, val_cache, mha_output, pos_ptr_));
  // 检测到这一层输出的误差比较大
...
void Qwen2Model::feed_forward(int32_t layer_idx, const tensor::Tensor& input) const {
  ...
  ffn_norm_output.reshape({prompt_len_, config_->dim_});
  ...
  w1_output.reshape({prompt_len_, config_->hidden_dim_});
  ...
  w3_ouput.reshape({prompt_len_, config_->hidden_dim_});
  ...
  // SwiGLU，输入 w1, w3 已经是 [seq_len, dim]，SwiGLU 是逐元素操作，输出写回 w1_output
  ...
  // reshape W2 输出
  w2_output.reshape({prompt_len_, config_->dim_});
...
void Qwen2Model::cls_logits(const tensor::Tensor& input) const {
  CHECK(qwen_layers_ != nullptr);
  int32_t seq_len = input.get_dim(0);
  int32_t dim = config_->dim_;

  // 构造指向最后一个 token 的 tensor view，指针移动到最后一行
  float* last_token_ptr = const_cast<float*>(input.ptr<float>()) + (seq_len - 1) * dim;
  // 形状强制为 [1, dim]
  tensor::Tensor last_token_input(base::DataType::kDataTypeFp32, 1, dim, false, nullptr, last_token_ptr);
  last_token_input.set_device_type(input.device_type());

  // 只对最后一个 token 做 RMSNorm，此处是 in-place 操作，会修改原 input 内存中最后一行的值
  ...
  // 确保输出形状是 [1, vocab_size]
  forward_output.reshape({1, config_->vocab_size_});
...
int32_t Qwen2Model::post_processing(const tensor::Tensor& pos, const std::vector<int32_t>& history,
                                    bool is_prompt) const {
    tensor::Tensor forward_output = get_buffer(ModelBufferType::kForwardOutput);
    size_t vocab_size = forward_output.size();
      return sampler_->sample(forward_output.ptr<float>(), vocab_size, history,
