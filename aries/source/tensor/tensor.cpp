...
static size_t reduce_dimension(T begin, T end, Tp init) {
  ...
  // 强制转换为 size_t 进行累乘
  size_t size = std::accumulate(begin, end, static_cast<size_t>(init), std::multiplies<size_t>());
  return size;
}
...
void Tensor::reshape(const std::vector<int32_t>& dims) {
  ...
  // 计算所需的字节数
  size_t needed_size = size * base::DataTypeSize(this->data_type_);
  // 只有当所需的字节数 > 当前 buffer 的物理容量时，才重新分配
  if (needed_size > buffer_->byte_size()) {
    auto new_buffer = std::make_shared<base::Buffer>(needed_size, buffer_->allocator());
    CHECK(new_buffer->allocate());
    // 需要保留数据则 copy，通常 reshape 用于输出 buffer，不一定要 copy
